...
function renderHtml(input: RegistrationEmailInput): string {
  ...
  // Primary CTA goes to WhatsOnChain (the only on-chain viewer surface we
  // expose). Falls back to the confirmation page when the mint stubbed or
  // failed — the button always resolves to something useful.
  const primaryHref = input.whatsOnChainUrl ?? input.confirmationUrl;
  const primaryLabel = input.whatsOnChainUrl
    ? "View your ticket on WhatsOnChain ↗"
    : "View your ticket";
