    ...
    <div className="grid lg:grid-cols-2 gap-6 mb-8">
      ...
      <GlassCard className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="text-bsva-blue text-xs font-display font-semibold uppercase tracking-wider mb-1">
              Server wallet
            </div>
            <div className="font-display font-semibold text-bsva-navy text-xl">
              {info?.enabled ? "Live" : info ? "Stub mode" : "…"}
            </div>
          </div>
          <button
            onClick={() => void refresh()}
            className="text-xs text-bsva-soft/50 hover:text-bsva-navy font-body"
            title="Refresh"
          >
            ↻ refresh
          </button>
        ...
        {info && (
          <dl className="grid grid-cols-[120px_1fr] gap-y-2.5 text-sm font-body">
            <dt className="text-bsva-soft/50">Status</dt>
            <dd className="text-bsva-navy">{info.status ?? "—"}</dd>
            <dt className="text-bsva-soft/50">Network</dt>
            <dd className="text-bsva-navy">{info.network ?? "—"}</dd>
            <dt className="text-bsva-soft/50">Basket</dt>
            <dd className="text-bsva-navy font-mono text-xs">{info.basket}</dd>
            <dt className="text-bsva-soft/50">Identity key</dt>
            <dd
              className="text-bsva-blue font-mono text-xs break-all"
              title={info.identityKey ?? undefined}
            >
              {info.identityKey ? truncateKey(info.identityKey, 12, 8) : "—"}
            </dd>
            <dt className="text-bsva-soft/50">Balance</dt>
            <dd className="text-bsva-navy font-display font-semibold">
              {formatSats(info.totalSatoshis)}{" "}
              {typeof info.utxoCount === "number" && (
                <span className="text-bsva-soft/50 font-body font-normal text-xs">
        ...
        {info?.enabled && info.lowBalance && (
          <div className="mt-5 rounded-lg border border-yellow-400/40 bg-yellow-400/[0.08] p-3 text-sm font-body">
            ...
            <div className="text-bsva-soft/70 leading-snug">
        ...
        {pendingMintCount > 0 && (
          <div className="mt-3 rounded-lg border border-bsva-cyan/30 bg-bsva-cyan/[0.06] p-3 text-sm font-body">
            <div className="text-bsva-blue font-display font-semibold mb-1">
        ...
        {pendingOrdCount > 0 && (
          <div className="mt-3 rounded-lg border border-bsva-cyan/30 bg-bsva-cyan/[0.06] p-3 text-sm font-body">
            ...
            <div className="text-bsva-soft/70 leading-snug">
              ...
              <span className="text-bsva-blue">Retry ord</span> on each row from the event's
              registrations list.
            </div>
          </div>
        )}
        ...
        {info && !info.enabled && (
          <div className="mt-5 rounded-lg border border-bsva-grey bg-white/[0.03] p-4 text-sm text-bsva-soft/70 font-body">
            <div className="text-bsva-navy font-display font-semibold mb-1">
              BSV mode is currently disabled.
            </div>
            Every registration receives a deterministic <span className="text-bsva-blue font-mono">stub-…</span> txid instead of a real on-chain ticket.
            <div className="mt-3">To switch to live mode:</div>
            <ol className="list-decimal pl-5 mt-1 space-y-1 text-bsva-soft/60">
              <li>
                Generate a wallet key:{" "}
                <code className="text-bsva-blue">npm --workspace server run bsv:generate-key</code>
              </li>
              <li>
                Paste the WIF into <code className="text-bsva-blue">.env</code> as{" "}
                <code className="text-bsva-blue">BSV_SERVER_PRIVATE_KEY</code>
              </li>
              <li>
                Set <code className="text-bsva-blue">BSV_ENABLED=true</code>
              </li>
        ...
        {infoLoading && !info && (
          <div className="text-bsva-soft/50 font-body text-sm">Loading…</div>
        )}
      </GlassCard>
      ...
      <GlassCard className="p-6">
        ...
        <div className="font-display font-semibold text-bsva-navy text-xl mb-4">BRC-29 transfer</div>

        {!wallet ? (
          <>
            <p className="text-sm text-bsva-soft/60 font-body mb-4 leading-relaxed">
            ...
            <p className="text-xs text-bsva-soft/50 font-body mb-4">
              Don't have one yet?{" "}
              <a
                href="https://hub.bsvblockchain.org/demos-and-onboardings/onboardings/onboarding-catalog/metanet-desktop-mainnet"
                target="_blank"
                rel="noreferrer"
                className="text-bsva-blue hover:text-bsva-navy transition-colors"
              >
            ...
            {!info?.enabled && (
              <p className="text-xs text-bsva-soft/40 font-body mt-3">
          ...
          <>
            <dl className="grid grid-cols-[120px_1fr] gap-y-2.5 text-sm font-body mb-5">
              <dt className="text-bsva-soft/50">Connected as</dt>
              <dd className="text-bsva-blue font-mono text-xs break-all" title={identityKey ?? undefined}>
            ...
            <form onSubmit={onFund} className="space-y-3">
              <label className="block">
                <span className="block text-xs font-body text-bsva-soft/60 mb-1">Amount (sats)</span>
                <input
                  type="number"
                  min={1}
                  step={1}
                  ...
                  className="w-full bg-bsva-grey/40 border border-bsva-grey rounded-lg px-3 py-2.5 text-bsva-navy font-body focus:outline-none focus:border-bsva-cyan focus:bg-bsva-grey transition-colors"
                />
              </label>
            ...
            {lastTxid && (
              <div className="mt-4 rounded-lg border border-bsva-cyan/30 bg-bsva-cyan/[0.06] p-3 text-sm font-body">
                ...
                <div className="font-mono text-xs text-bsva-soft/80 break-all">{lastTxid}</div>
                <div className="text-bsva-soft/50 text-xs mt-2">
